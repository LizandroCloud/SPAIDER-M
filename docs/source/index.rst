Welcome to SPAIDER documentation
===================================

**Sequential and Parallel Algorithms Integrated to Differential Equation Routines**

Contents
--------

.. toctree::

   Introduction
   Installation
   Quick_Start
   Dynamic Optimization
   Toolbox Description
   Running with Singles Shooting
   Running with Multiple Shooting
   Running with Collocation
   SPAIDER Hysys Examples
   SPAIDER Aspen Plus Examples
   SPAIDER Emso Examples
   SPAIDER Ansys CFX
   SPAIDER Python Integration
   SPAIDEER Interface with ScadaBR
   Conclusions
